...
class Tree {
    ...
    auto releaseSubTree(std::unique_ptr<Node> n) -> void;
    ...
    auto tryLocateNode(const std::optional<KeyT>& node) const -> Node*;
...
public:
    ...
    Tree<KeyT, PayloadT> subTree(const KeyT& key) const;
    ...
    auto addSubtree(const Tree<KeyT, PayloadT>& addedTree,
                    const std::optional<KeyT>& parent,
    ...
    static auto unflatten(std::span<const std::optional<entry_t>> flat) -> Tree;
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
Tree<KeyT, PayloadT>::Tree() = default;
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::addChild(KeyT key,
                                    PayloadT payload,
                                    const std::optional<KeyT>& parent,
                                    std::optional<int64_t> insertBeforePosition)
    -> void
{
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::addSubtree(
    const Tree<KeyT, PayloadT>& addedTree,
    const std::optional<KeyT>& parent,
    const std::optional<int64_t>& insertBeforePosition) -> void
{
    int64_t pos{insertBeforePosition.value_or(0)};

    addedTree.for_each([&](auto level, auto* node) {
        if (level == 0) {
            addChild(node->key, node->payload, parent, pos++);
        }
        else {
            addChild(node->key, node->payload, addedTree.parent(node->key));
        }
    });
}
    ...
    -> Tree<KeyT, TransformResultT<Func>>
{
    ...
    auto transformPayload = [&](auto level, auto* node) {
        ...
        mappedTree.addChild(node->key, func(node->payload), nodeParent);
    };
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::keysView() const
{
    return std::views::keys(registry);
}

template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::payloadView() const
{
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::entriesView() const
{
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::parent(const KeyT& child) const
    -> std::optional<std::reference_wrapper<const KeyT>>
{
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::payload(const KeyT& key) const
    -> std::optional<std::reference_wrapper<const PayloadT>>
{
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::children(const KeyT& key) const
{
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::children() const
{
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::nthChild(const KeyT& key, size_t n) const
    -> std::optional<std::reference_wrapper<const PayloadT>>
{
    if (auto it = registry.find(key); it != cend(registry)) {
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::nthChild(size_t n) const
    -> std::optional<std::reference_wrapper<const PayloadT>>
{
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::positionInChildren(const KeyT& key) const
    -> std::optional<size_t>
{
    auto it = registry.find(key);
    if (it == cend(registry)) {
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::leaves() const -> std::vector<PayloadT>
{
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::flatten() const
...
// static //
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::unflatten(
    std::span<const std::optional<entry_t>> flat) -> Tree
{
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::hasNode(const KeyT& node) const -> bool
{
    return registry.contains(node);
}

template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::removeNodes(const std::optional<KeyT>& parent,
                                       int64_t row,
                                       int64_t count) -> void
{
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::removeNode(const KeyT& key) -> void
{
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::moveNodes(
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::display() const -> std::string
{
    std::stringstream ss;
...
auto entriesDFS(const Tree<K, P>& tree)
    -> std::vector<typename Tree<K, P>::entry_t>
{
    std::vector<typename Tree<K, P>::entry_t> result;
    tree.dfs(
...
template <typename K, typename P>
auto operator==(const Tree<K, P>& lhs, const Tree<K, P>& rhs) -> bool
{
...
template <typename K,
          typename P,
          typename Comp = std::equal_to<typename Tree<K, P>::entry_t>>
auto compare(const Tree<K, P>& lhs, const Tree<K, P>& rhs, Comp comp = Comp{})
    -> bool
{
    const auto leftEntries = entriesDFS(lhs);
    const auto rightEntries = entriesDFS(rhs);
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::releaseSubTree(std::unique_ptr<Node> n) -> void
{
    if (not n) {
        // Root pointer might not manage memory when Tree is in `moved from`
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
template <typename Func>
auto Tree<KeyT, PayloadT>::for_each(Func func, const Node* initial) const
    -> void
{
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::addChildInternal(std::unique_ptr<Node> child,
                                            const std::optional<KeyT>& parent,
                                            int64_t insertBeforePosition)
    -> void
{
    auto* parentPtr = tryLocateNode(parent);
...
template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::removeNodesInternal(
    const std::optional<KeyT>& parent, int64_t row, int64_t count) -> void
{
    ...
    children.erase(first, last);
    for (auto& node : nodes) {
        releaseSubTree(std::move(node));
    }
}

template <std::default_initializable KeyT, std::default_initializable PayloadT>
auto Tree<KeyT, PayloadT>::tryLocateNode(const std::optional<KeyT>& node) const
    -> Node*
{
    auto nodePtr = [&](const auto& key) {
        if (auto it = registry.find(key); it == cend(registry)) {
            std::stringstream ss;
