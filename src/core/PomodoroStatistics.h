...
public:
    ...
    QVector<Slice> getSortedSliceVector() const { return sliceData; }
...
private:
        ...
        transform(sliceData.begin(), sliceData.end(), sliceData.begin(),
            [total](const auto& entry) {
                return std::make_pair(
                    entry.first, double(entry.second) / total);
            });
        sort(sliceData.begin(), sliceData.end(),
    ...
    void reduceTailToSum()
    {
        ...
        QSet<QString> allTags
            = QSet<QString>::fromList(tagToPomodoroVec.keys());
...
public:
    PomodoroStatItem(
        const QVector<Pomodoro>& pomodoros, const DateInterval& dateInterval)
        : interval(dateInterval)
        , pomodoros(pomodoros)
    {
        dailyDistribution
            = new Distribution<double>{computeDailyDistribution()};
        weekdayDistribution = new Distribution<double>{
            computeWeekdayDistribution(), countWeekdays()};
    ...
    QVector<Pomodoro> getPomodoros() const { return pomodoros; }
