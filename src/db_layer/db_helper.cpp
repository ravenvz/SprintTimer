...
bool createSchema() {
    ...
    QString createPomodoroTable = "CREATE TABLE pomodoro "
                                  "(id INTEGER PRIMARY KEY AUTOINCREMENT, "
                                  "todo_id INTEGER, "
                                  "start_time DATETIME, "
    ...
    QString createCleanOrphanedTagTrigger = "CREATE TRIGGER on_todotag_delete "
    ...
    // Trigger to increment spent pomodoros in todo_item when new pomodoro is inserted
    QString createIncrementSpentTrigger = "CREATE TRIGGER increment_spent_after_pomo_insert "
    ...
    // Trigger to decrement spent pomodoros in todo_item after pomodoro is removed
    QString createDecrementSpentTrigger = "CREATE TRIGGER decrement_spent_after_pomo_delete "
    ...
    // View for pomodoros
    QString createPomodoroView = "CREATE VIEW pomodoro_view AS "
    ...
    QString createPomodoroViewDeleteTrigger = "CREATE TRIGGER delete_from_pomodoro_view "
                                              "INSTEAD OF DELETE ON pomodoro_view "
                                              "BEGIN "
                                              "DELETE FROM pomodoro WHERE id = old.id; "
                                              "END;";

    QString createPomodoroViewInsertTrigger = "CREATE TRIGGER instead_pomodoro_view_insert "
                                              "INSTEAD OF INSERT "
                                              "ON pomodoro_view "
                                              "BEGIN "
    ...
    // View for todo items
    QString createTaskView = "CREATE VIEW task_view AS "
                             "SELECT todo_item.id, "
                             "todo_item.name, "
                             "estimated_pomodoros, "
                             "spent_pomodoros, "
    ...
    QString createTaskViewDeleteTrigger = "CREATE TRIGGER on_task_view_delete "
    ...
    QString createTaskViewUpdateTrigger = "CREATE TRIGGER on_task_view_update "
...
bool createDbConnection() {
    ...
    if (!databaseFile.exists()) {
        ...
        if (!createDatabase(db, filename)) return false;
    } else {
