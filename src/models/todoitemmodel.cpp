    ...
    SqliteTableModel(parent)
{
    ...
    insertTodoItemQuery.prepare("insert into todo_item (name, estimated_pomodoros, spent_pomodoros, completed, priority, last_modified) "
    ...
    createRelationQuery.prepare("insert into todotag (tag_id, todo_id) values (:tag_id, :todo_id);");
    removeRelationQuery.prepare("delete from todotag where todo_id = (:todo_id) and "
...
Qt::ItemFlags TodoItemModel::flags(const QModelIndex &index) const {
    ...
    return QSqlTableModel::flags(index) | Qt::ItemIsUserCheckable | Qt::ItemIsDragEnabled;
}
...
bool TodoItemModel::insert(const TodoItem& item) {
    ...
    if (!insertTodoItemQuery.exec()) shouldRollback = true;
...
bool TodoItemModel::toggleCompleted(const QModelIndex& index) {
    ...
    bool timeStampUpdated = QSqlTableModel::setData(
            index.model()->index(index.row(), static_cast<int>(Column::LastModified)),
...
bool TodoItemModel::replaceItemAt(const int row, const TodoItem& newItem) {
    ...
    if (oldTags != newTags) {
        ...
        if (!updateTags(row, tagsToRemove, tagsToInsert)) return false;
    }
                            ...
                            const QModelIndex& destinationParent, int destinationChild)
{
    ...
    int destinationRow = (destinationChild == -1) ? rowCount() - 1 : destinationChild;
...
bool TodoItemModel::setItemPriority(const int row, const int priority) {
    return QSqlTableModel::setData(index(row, static_cast<int>(Column::Priority)),
                                   priority,
                                   Qt::EditRole);
}

QVariant TodoItemModel::columnData(const QSqlRecord& rowRecord, const Column& column) const {
...
bool TodoItemModel::updateTags(const int row, const QSet<QString>& tagsToRemove, const QSet<QString>& tagsToInsert) {
    ...
    QSqlDatabase::database().transaction();
    bool shouldRollback = !(insertTags(todoId, tagsToInsert) && removeTags(todoId, tagsToRemove));
    if (shouldRollback || !QSqlDatabase::database().commit()) {
        QSqlDatabase::database().rollback();
        return false;
    }
...
bool TodoItemModel::removeTags(QVariant itemId, const QSet<QString>& tags) {
    ...
    for (const auto& tag : tags) {
        ...
        if (!removeRelationQuery.exec()) success = false;
    }
    return success;
}
...
bool TodoItemModel::updateRow(const int row, const TodoItem& newItem) {
    ...
    bool timestampUpdated = QSqlTableModel::setData(
            ...
            Qt::EditRole);
    if (nameUpdated && estimatedUpdated && spentUpdated && completedUpdated && timestampUpdated) {
        submitAll();
        return true;
    };
    revertAll();
