...
void Plot::constructPointBoxes() {
    ...
    const double scaleX = rangeX.span() > 0 ? availableRect.width() / (rangeX.span()) : 1;
    const double scaleY = rangeY.span() > 0 ? (availableRect.height() - labelOffset) / (rangeY.span()) : 1;
    for (int graphNum = 0; graphNum < graphs.size(); ++graphNum) {
